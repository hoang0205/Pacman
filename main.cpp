...
bool InitData()
{
    ...
    font = TTF_OpenFont("font//dlxfont.ttf", 12);
    if (font == NULL)
    {
...
int main(int argc, char* argv[])
{
    ...
    {
        fps_timer.start();
        //gPNGSurface = SDL_Common::loadSurface("img//menu.png");
        ...
        std::string score_val = std::to_string(p_player.Point * 100);
        str_score += score_val;

        time_game.SetText(str_time);
        time_game.loadFromRenderedText(font, g_screen);
        time_game.RenderText(g_screen, 393, 393);

        Score.SetText(str_score);
        Score.loadFromRenderedText(font, g_screen);
        Score.RenderText(g_screen, SCREEN_WIDTH - 400, 393);
